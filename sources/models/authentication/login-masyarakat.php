...
<body class="hold-transition login-page" id="body-theme">
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $result = mysqli_query($connection, "SELECT id FROM masyarakat WHERE username='$username' AND password='$password';");
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      echo "<script>successModal('', '/$originalPath', 'login-container');</script>";
    } else {
      echo "<script>errorModal('Username atau password salah', '', 'login-container');</script>";
    }
  };
  ?>
</body>
